...
def create_payoff_tool(num_agents, network: TrafficNetwork):
    @tool
    def calculate_payoff(route_distribution: dict, chosen_route: str) -> int:
        ...
        if chosen_route not in route_distribution or route_distribution[chosen_route] < 1:
            raise ValueError(f"The chosen route '{chosen_route}' must have at least one player. This is because you yourself are traveling on the route.")
