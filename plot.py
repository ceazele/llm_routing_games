...
# Plotting functions
def plot_routes(data, output_folder, bridge):
    if bridge:
        route_key = "with_bridge_routes"
        valid_routes_key = "valid_routes_with_bridge"
        title = "Number of Subjects per Route (With Bridge)"
        filename = "with_bridge_routes.png"
    else:
        route_key = "no_bridge_routes"
        valid_routes_key = "valid_routes_no_bridge"
        title = "Number of Subjects per Route (No Bridge)"
        filename = "no_bridge_routes.png"

    all_routes = pd.concat(data[route_key], keys=range(len(data[route_key])), axis=1)
    ...
    se_routes = (
        ...
        .T
        .groupby(level=1)
        .apply(lambda x: x.std(ddof=1) / np.sqrt(len(data[route_key])))
        .T
    )

    plt.figure(figsize=(12, 6))
    for route in data[valid_routes_key]:
        if route in mean_routes.columns:
            mean_values = mean_routes[route]
            se_values = se_routes[route]
            rounds = mean_routes.index
