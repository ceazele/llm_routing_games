...
At the end of each round, you will be informed of the number of drivers who chose each route, your payoff for that round, and the payoff you would have earned had you chosen each alternative route. 
...
def prompt_route(session_id, num_rounds, num_agents, num_routes, demographics, description, avail_routes, has_bridge, prev_routes, costs_dict, chosen_route):
    ...
    messages = []
    if prev_routes is None and costs_dict is None:
        ...
        # The number of players on each segment was: {prev_segment}\n
        
        # Parse costs from the dictionary
        chosen_route_cost = costs_dict[chosen_route]
        alternative_payoffs = []

        for route, cost in costs_dict.items():
            if route != chosen_route:
                alternative_payoff = f"If you had chosen {route}, your payoff would have been {400 - cost}"
                alternative_payoffs.append(alternative_payoff)

        # Create the message for the agent, including alternative payoffs
        message_content = f"Last round, the number of players on each route was: {prev_routes}\n" \
                          f"Your payoff was {400 - chosen_route_cost}\n" + \
                          "\n".join(alternative_payoffs)
    ...
    with open(filename, mode='w', newline='') as file:
        ...
        for round_num in range(1, num_rounds + 1):
            ...
            for session_id in range(0, num_agents):
                ...
                if round_num == 1:
                    decision = prompt_route(session_id, num_rounds, num_agents, len(str_routes), demographics, description, str_routes, has_bridge, None, None, None)
                else:
                    decision = prompt_route(session_id, num_rounds, num_agents, len(str_routes), demographics, description, str_routes, has_bridge, prev_routes, prev_costs[session_id], agent_routes[session_id])
            ...
            for session_id in range(0, num_agents):
                ...
                for route in str_routes:
                    if route == chosen_route:
                        # The agent already chose this route, no adjustment needed
                        cost = calculate_total_cost(path_to_edges(route), G)
                        writer.writerow([round_num, session_id, chosen_route, cost])
                        print(f"Agent {session_id} chose route {chosen_route} with cost {cost}")
                    else:
                        # Adjust player counts: -1 on chosen route, +1 on this alternative route
                        add_player_to_path(path_to_edges(route), G)
                        remove_player_from_path(path_to_edges(chosen_route), G)
                        cost = calculate_total_cost(path_to_edges(route), G)
                        # Revert the adjustments
                        add_player_to_path(path_to_edges(chosen_route), G)
                        remove_player_from_path(path_to_edges(route), G)

                    prev_costs[session_id][route] = cost
...
run_simulation(True, 10, 18, demographics, 'routes_with_bridge_10_18.csv')
